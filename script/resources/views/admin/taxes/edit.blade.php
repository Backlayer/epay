...
@section('content')
    <div class="row justify-content-center">
        <div class="col-md-6">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('admin.taxes.update', $tax->id) }}" method="post" class="ajaxform_with_redirect">
                        @csrf
                        @method('PUT')

                        <div class="form-group">
                            <label for="name" class="required">{{ __('Name') }}</label>
                            <input type="text" name="name" id="name" class="form-control" value="{{ $tax->name }}" placeholder="{{ __('Enter tax name') }}" required>
                        </div>

                        <div class="form-group">
                            <label for="rate" class="required">{{ __('Rate') }} <span id="taxType"></span></label>
                            <input type="number" name="rate" id="rate" class="form-control" value="{{ $tax->rate }}" placeholder="{{ __('Enter tax rate') }}" required>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="status" id="status" class="form-control" data-control="select2" required>
                                ...
                                <option value="0"  @selected($tax->status == '0')>{{ __('Inactive') }}</option>
                            </select>
                        </div>
